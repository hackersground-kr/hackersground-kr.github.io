...
발행 워크플로우는 해당 경로의 정적 리다이렉트 페이지를 함께 만들고, 배포 분석 태그 주입 대상에서도 이 페이지를 제외합니다. 따라서 단축 URL 접속은 최종 상세 페이지에서만 Google Analytics 페이지뷰로 집계됩니다. 상세 상단의 **공유하기** 버튼은 이 단축 URL을 Web Share API 또는 클립보드로 전달합니다.
