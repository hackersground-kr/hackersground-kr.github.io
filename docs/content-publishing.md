...
1. 메타데이터(`Slug`, 제목, 한 줄 요약, 이모지, 태그)는 발행 이슈의 전용 필드에만 작성합니다.
2. 본문은 소제목 전에 자연스러운 도입 문단을 두 개 이상 작성합니다.
3. 이후 `## 소제목`으로 이야기를 전개합니다.

발행 스크립트는 위 필수 메타데이터와 본문 구조를 검증합니다. 형식이 맞지 않으면 발행하지 않으므로, 메타데이터가 본문에 노출되거나 소제목부터 갑자기 시작하는 글을 막을 수 있습니다.
...
발행 워크플로우는 해당 경로의 정적 리다이렉트 페이지를 함께 만들고, 배포 분석 태그 주입 대상에서도 이 페이지를 제외합니다. 따라서 단축 URL 접속은 최종 상세 페이지에서만 Google Analytics 페이지뷰로 집계됩니다. 리다이렉트 페이지에는 콘텐츠 제목과 한 줄 요약을 Open Graph 메타데이터로 넣어, 메신저 미리보기에도 상세 콘텐츠 정보가 표시됩니다. 상세 상단의 **공유하기** 버튼은 단축 URL만 클립보드로 복사합니다.

기존 콘텐츠에는 **Migrate short links** 워크플로우를 수동 실행해 번호와 정적 단축 경로를 생성합니다. 콘텐츠 삭제 워크플로우는 DB 레코드, 목록 메타데이터, 단축 경로를 함께 제거합니다. 마이그레이션 API는 인증된 워크플로우에서만 `/api/content/{kind}/admin/short-id-migration`으로 호출합니다.
